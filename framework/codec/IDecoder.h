...
namespace Sivin {
  ...
  public:
    virtual SNRetStatus open(const std::unique_ptr<SNStreamInfo> &streamInfo, void *surface, uint64_t flags) = 0;

    virtual void close() = 0;

    virtual SNRetStatus sendPacket(std::unique_ptr<SNPacket> &packet, uint64_t timeout) = 0;

    virtual SNRetStatus getFrame(std::unique_ptr<SNFrame> &frame, uint64_t timeout) = 0;
