...
#define CHECK_DEMUXER do{if (mDemuxer == nullptr) return -1;}while(false);
#define CHECK_DEMUXER_V do{if (mDemuxer == nullptr) return;}while(false);
...
namespace Sivin {
    ...
    int DemuxerService::createDemuxer(IDemuxer::DemuxerType demuxerType) {
        if (mDemuxer == nullptr) {
            ...
            mDemuxer = DemuxerFactory::createDemuxer(url);
